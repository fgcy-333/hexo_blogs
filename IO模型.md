...
大多数读者在学习网络通信相关技术的时候，都只是接触到零碎的通信技术点，没有完整的技术体系架构，以至于对于Java的通信场景总是没有清晰的解决方案。本次课程将通过大量清晰直接的案例从最基础的BIO式通信开始介绍到NIO , AIO，读者可以清晰的了解到阻塞、同步、异步的现象、概念和特征以及优缺点。
...
![image-20200615181255063](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212032341670.png)
...
![image-20200615180441015](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022224468.png)
...
<img src="https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022232008.png" alt="image-20200618222916021" style="zoom: 67%;" />
...
客户端Socket负责发起连接操作。连接成功后，双方通过输入和输出流进行同步阻塞式通信。 
...
public class ClientDemo {
    public static void main(String[] args)  throws Exception {
        ...
        // （4）开始发送消息
        ps.println("我是客户端，我想约你吃小龙虾！！！");//这里要使用换行，因为服务器使用换行符作为一次数据传输的结束符，如果不使用换行，服务器会认为还没		// 有发送完一次数据，会继续等待，而客户端会继续运行，然后结束程序 此时心跳机制发现双方断开连接 于是服务器报错
        ps.flush();
    }
}
```
...
public class ServerDemo {
    public static void main(String[] args) throws Exception {
        ...
        //（5）读取数据
        String line ;
        while((line = br.readLine())!=null){//到这一步，tcp连接已建立 因为这里使用while 服务器会不断等待接收客户端的字节流直到tcp断开连接(底层会有心跳机制)
            //当客户端程序结束 心跳机制发现双方断开连接时 客户端会报错
...
![image-20221202224835101](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022248635.png)
...
![image-20221202225809539](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022258623.png)
...
![image-20221202230736171](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022307544.png)

![image-20221202230745761](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022307614.png)
...
![image-20200615181141593](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212022310050.png)
...
public class ServerThread extends Thread {
    ...
    public void run() {
        try {
            ...
            String msg;
            while ((msg = br.readLine()) != null) {
                System.out.println("服務器接收到" + socket.getPort() + "的消息：" + msg);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
...
![image-20221203235725618](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212032357255.png)
...
![image-20221203235142119](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212032351045.png)

---

![image-20221203235119631](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212032351499.png)
...
![image-20200619085953166](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212040014282.png)
...
public class ServerDemo {
    public static void main(String[] args) throws IOException {
        System.out.println("=======server======");
        ServerSocket ss = new ServerSocket(5555);
        //核心线程数2个，救急线程数0个，任务队列中只能放一个 这个线程池可以同时运行三个任务 超过三个就会报错
        ExecutorService threadPool = new ThreadPoolExecutor(2, 2, 30,
                TimeUnit.SECONDS, new ArrayBlockingQueue<>(1), Executors.defaultThreadFactory(), new ThreadPoolExecutor.AbortPolicy());
        while (true) {
            Socket accept = ss.accept();
            threadPool.execute(new ServerTarget(accept));
        }
    }
}
~~~~
...
![image-20221204003950751](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212040039163.png)
...
![image-20221204002537562](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212040026489.png)

----

<img src="https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212040027247.png" alt="image-20221204002655444" style="zoom:67%;" />
...
public class Client {
    public static void main(String[] args) throws IOException {
        ...
        File file = new File("D:\\GithubBlog\\source\\_posts\\spring-security.md");
...
public class TargetRunner extends Thread {
    ...
    public TargetRunner(Socket socket) {
        this.socket = socket;
    }
    ...
    public void run() {
        try {
            ...
            FileOutputStream fos = new FileOutputStream("D:\\learn\\project\\xmlDemo\\" + name);
            byte[] buffer = new byte[1024];
            int len = 0;
...
![image-20200619123304241](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212062325299.png)
...
![image-20200223212913139](D:/learn/黑马程序员/IO模型/讲义/BIO、NIO、AIO.assets/image-20200223212913139.png)
...
* 2.其次，点击客户端类ClientChat类，在弹出的方框中输入服务端的ip和当前客户端的昵称
  ...
  ![image-20200223214123052](D:/learn/黑马程序员/IO模型/讲义/BIO、NIO、AIO.assets/image-20200223214123052.png)
...
* 如果选中右侧在线列表某个用户，默认发送@消息
  ...
  * 如果选中右侧在线列表某个用户，然后选择右下侧私聊按钮默，认发送私聊消息。
  ...
  ![image-20200223214143465](D:/learn/黑马程序员/IO模型/讲义/BIO、NIO、AIO.assets/image-20200223214143465.png)

  ![image-20200223214155975](D:/learn/黑马程序员/IO模型/讲义/BIO、NIO、AIO.assets/image-20200223214155975.png)
...
* ![image-20200223232406727](D:/learn/黑马程序员/IO模型/讲义/BIO、NIO、AIO.assets/image-20200223232406727.png)
* 如果直接点击发送，默认发送群聊消息
* 如果选中右侧在线列表某个用户，默认发送@消息
* 如果选中右侧在线列表某个用户，然后选择右下侧私聊按钮默，认发送私聊消息。
...
![image-20200619153658139](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212072235555.png)
...
![image-20200619163952309](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212072235841.png)
...
<img src="https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212072236636.png" alt="image-20200619172434538" style="zoom:67%;" />
...
~~~java
 @Test
    public void aaa() {
        //分配缓冲区
        ByteBuffer buffer = ByteBuffer.allocate(10);//分配容量为10的缓冲区
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:0
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:10
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10

        //往缓冲区中放值
        buffer.put("waiting".getBytes());
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:7
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:10
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10

        //读取模式:将缓冲区中的终点位置设置为当前位置，然后将当前位置设置为0
        buffer.flip();
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:0
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:7
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10

        //读取一个字符
        char ch = (char) buffer.get();
        System.out.println(ch);//w
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:1
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:7
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10
...
~~~java
    ...
    public void bbb() {
        ...
        buffer.put("waiting".getBytes());
        buffer.flip();//不开启读取模式，就读不到数据
        byte[] bytes = new byte[2];
        buffer.get(bytes);
        System.out.println(new String(bytes));//wa
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:2
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:7
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10
        //标记当前位置
        buffer.mark();//2
        ...
        byte[] b = new byte[3];
        buffer.get(b);
        System.out.println(new String(b));//iti
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:5
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:7
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10
        ...
        if (buffer.hasRemaining()) {
            ...
            System.out.println("剩下：" + remain);//剩下：5
        }
        byte[] by = new byte[remain];
        System.out.println("当前缓冲区的起始位置:" + buffer.position());//当前缓冲区的起始位置:2
        buffer.get(by);
        System.out.println(new String(by));//iting
        System.out.println("当前缓冲区的终点位置:" + buffer.limit());//当前缓冲区的终点位置:7
        System.out.println("当前缓冲区的容量:" + buffer.capacity());//当前缓冲区的容量:10
    }
~~~
...
很明显，在做IO处理时，比如网络发送大量数据时，直接内存会具有更高的效率。直接内存使用allocateDirect创建，但是它比  **申请**  普通的堆内存需要耗费更高的性能。不过，这部分的数据是在JVM之外的，因此它不会占用应用的内存。
...
~~~java
    ...
    public void ccc() {
        ...
        System.out.println("buffer1是否是直接内存:" + buffer1.isDirect());//false
        System.out.println("buffer2是否是直接内存:" + buffer2.isDirect());//true
    }
~~~
...
<img src="https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212082148711.png" alt="image-20221208214842830" style="zoom:67%;" />
...
```java
    ...
    public void eee() throws IOException {
        ...
        System.out.println(str);//使用NIO写文件
    }
```
...
~~~java
    ...
    public void ggg() throws IOException {
        ...
        FileOutputStream fos = new FileOutputStream("b.txt");//当前项目路径下
        ...
        Arrays.stream(buffers).forEach(obj->obj.flip());
...
![image-20200619230246145](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212082242799.png)
...
![image-20200619153658139](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202212082350524.png)
...
- NIO方式适用于连接数目多且连接比较短（轻操作）的架构，比如聊天服务器，并发局限于应用中，编程比较复杂，JDK1.4开始支持。Netty是基于NIO的
