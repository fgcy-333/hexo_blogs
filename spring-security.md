...
date: 2022-12-05 23:45:34
tags:
- SpringSecurity
- 三更
categories: 
- 视频学习笔记
index_img: /images/springsecurity.png
---
...
​	而 `认 证 ` 和 `授权` 也是SpringSecurity作为安全框架的核心功能。
...
**ExceptionTranslationFilter：**处理过滤器链中抛出的任	`AccessDeniedException	` 和	`AuthenticationException `。
...
> 获取spring容器，然后查看里面的bean     container.getBean(DefaultSecurityFilterChain.class)
...
​	在接口中我们通过	`AuthenticationManager`	的 `authenticate` 方法来进行用户认证,所以需要在SecurityConfig中配置把AuthenticationManager注入容器。
