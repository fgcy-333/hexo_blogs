...
SQL标准是由`ANSI`制定的；不同数据库管理系统对其实现有所不同；这些  **不  同称为方言**

---

![](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/SQLisputonghua.jpg)
...
![](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/image-20211014230114639.png)
...
- 重命名一个列
- 便于计算
-    紧跟列名   ，也可以   **在列名和别名之间   加入关键字  AS，别名使用  双引号  **，以便``。
- `AS `可以省略
- 建议别名简短，见名知意
- 举例
...
- 所有  **运算符**  或  **列**  值遇到null值，运算的结果都为null
...
- Key：表示该列  **是否已编制索引**。PRI表示该列是表主键的一部分；UNI表示该列是UNIQUE索引的一部分；`MUL`表示在列中  **某个给定值**  **允许出现多次**。
