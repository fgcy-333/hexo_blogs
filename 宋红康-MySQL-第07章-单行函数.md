...
1、我们在使用 SQL 语言的时候，不是直接和这门语言打交道，而是通过它与不同的数据库软件进行交互，即 书写SQL语言与DBMS进行交互
...
这里将这些丰富的内置函数再分为两类：`单行函数`、`聚合函数（或分组函数）`
...
DATE_ADD('2021-10-21 23:32:12',INTERVAL '1_1' MINUTE_SECOND) AS col4, -- 加一分钟一秒
DATE_ADD(NOW(), INTERVAL -1 YEAR) AS col5, #可以是负数
DATE_ADD(NOW(), INTERVAL '1_1' YEAR_MONTH) AS col6 -- 加一年一个月 需要单引号
FROM DUAL;
```
...
+---------------------+---------------------+------------------------+------------------------------+---------------------------------------+----------------+-----------------------+-----------------+--------------------------+--------------------+-------------------------------+
| 2022-12-31 17:01:50 | 2022-12-31 17:01:00 | 2022-12-31 16:00:27    |                          456 | 838:59:59                             | 0001-01-01     |                   359 | 2022-12-31      | 2022-01-12               | 10:21:23           |                20200101010111 |
+---------------------+---------------------+------------------------+------------------------------+---------------------------------------+----------------+-----------------------+-----------------+--------------------------+--------------------+-------------------------------+
1 row in set, 1 warning (0.00 sec)
```

```mysql
mysql> SELECT ADDTIME(NOW(), 50),now();
+---------------------+---------------------+
| ADDTIME(NOW(), 50)  | now()               |
+---------------------+---------------------+
| 2022-12-31 17:03:09 | 2022-12-31 17:02:19 |
+---------------------+---------------------+
1 row in set (0.00 sec)
```

~~~mysql
mysql> SELECT ADDTIME(NOW(),'1:1:1'),now();
+------------------------+---------------------+
| ADDTIME(NOW(),'1:1:1') | now()               |
+------------------------+---------------------+
| 2022-12-31 18:04:53    | 2022-12-31 17:03:52 |
+------------------------+---------------------+
1 row in set (0.00 sec)
~~~
...
```mysql
mysql> SELECT SUBTIME(NOW(), '1:1:1'),now();
+-------------------------+---------------------+
| SUBTIME(NOW(), '1:1:1') | now()               |
+-------------------------+---------------------+
| 2022-12-31 16:03:45     | 2022-12-31 17:04:46 |
+-------------------------+---------------------+
1 row in set (0.00 sec)
```
...
| SUBTIME(NOW(), '-1:-1:-1') | now()               |
+----------------------------+---------------------+
| 2022-12-31 17:05:07        | 2022-12-31 17:05:06 |
+----------------------------+---------------------+
1 row in set, 1 warning (0.00 sec)
~~~~
...
| 函数                        | 用法                                                         |
| --------------------------- | ------------------------------------------------------------ |
| PASSWORD(str)               | 返回字符串str的加密版本，41位长的字符串。加密结果`不可逆`，常用于**用户的密码加密** |
...
~~~mysql
mysql> select substr(last_name,1,1) 首字母,CHAR_LENGTH(last_name) 姓长度 from employees order by 首字母;
+--------+--------+
| 首字母  |  姓长度 |
+--------+--------+
| A      |      6 |
