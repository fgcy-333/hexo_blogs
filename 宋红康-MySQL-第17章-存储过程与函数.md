...
存储过程和函数能够将复杂的SQL逻辑封装在一起，应用程序无须关注存储过程和函数内部复杂的SQL逻辑，而只需要 **简单地调用存储过程和函数** 即可。
...
执行过程：存储过程预先存储在 MySQL 服务器上且该存储过程已经编译好了，需要执行的时候，客户端只需要向服务器端发出调用存储过程的命令，服务器端就可以把预先存储好的这一系列 SQL 语句全部执行。
...
不过它和视图不同，视图是`虚拟表`，一般不会对视图进行增删改的操作，即通常不对底层数据表直接操作
...
**![image-20220926002532107](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202209260025499.png)**
...
![image-20220926002739921](https://cdn.jsdelivr.net/gh/fgcy-333/gitnote-images/2022/8/27202209260027007.png)
